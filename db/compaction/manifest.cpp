...
bool IsL1ManifestTableTrackingEnabled() {
    // Phase 2 default: disable L1 per-PST manifest slots on the hot path.
    return ParseEnvBool("FLOWKV_L1_MANIFEST_TRACK_TABLES", false);
}
...
Manifest::Manifest(int fd_, bool recover)
    : fd(fd_),
      ...
      end_(ManifestSize),
      track_l1_tables_(IsL1ManifestTableTrackingEnabled())
{
...
Version *Manifest::RecoverVersion(Version *version, SegmentAllocator *allocator)
{
    ...
    bool recovered_l1_from_manifest_tables = false;
    bool recovered_l1_from_snapshot = false;

    if (!track_l1_tables_ && has_l1_snapshot) {
        // Tableless mode: recover L1 directly from persisted hybrid state.
        ...
        if (version->ImportL1HybridState(l1_hybrid_state_bytes, current_L1_version)) {
            ...
            recovered_l1_from_snapshot = true;
        } else {
            INFO("L1 hybrid state snapshot invalid for seq=%u, fallback to manifest tables",
                 current_L1_version);
            ClearL1HybridState();
        }
    }

    if (!recovered_l1_from_snapshot) {
        INFO("recovering L1: %lu entries (%lu pages)%s",
             tail,
             (tail + metas_per_page - 1) / metas_per_page,
             track_l1_tables_ ? "" : " [compat manifest fallback]");
        // l1_tail includes reclaimed/invalid slots. Avoid reserving by tail here,
        // otherwise sparse manifests can trigger oversized hash-bucket allocation.
        version->BeginRecoverLevel1(current_L1_version + 1, 0);
        ...
        for (size_t i = 0; i < tail; i++)
        {
            ...
            if (meta.datablock_ptr_ != 0)
            {
                ...
                else
                {
                    ...
                    version->RecoverLevel1Table(tmeta);
                }
            }
        }
        version->FinalizeRecoverLevel1();
        recovered_l1_from_manifest_tables = true;

        if (has_l1_snapshot) {
            if (version->ImportL1HybridState(l1_hybrid_state_bytes, current_L1_version)) {
                INFO("recover L1 hybrid state from manifest snapshot, seq=%u", current_L1_version);
            } else {
                INFO("L1 hybrid state snapshot invalid for seq=%u, fallback to rebuilt tree",
                     current_L1_version);
                ClearL1HybridState();
            }
        }
    ...
    if (recovered_l1_from_manifest_tables) {
...
void Manifest::PrintL1Info() {
    std::lock_guard<std::recursive_mutex> guard(manifest_mu_);
	printf("[Manifest] l1 tail=%lu, l1_freelist=%lu, track_l1_tables=%s\n",
           CurrentSuper().l1_tail,
           l1_freelist_.size(),
           track_l1_tables_ ? "true" : "false");
}

int Manifest::AddTable(PSTMeta meta, int level)
{
    std::lock_guard<std::recursive_mutex> guard(manifest_mu_);
    if (level == 1 && !track_l1_tables_) {
        return -1;
    }
    int idx = -1;
...
{
    std::lock_guard<std::recursive_mutex> guard(manifest_mu_);
    if (level == 1 && (!track_l1_tables_ || idx < 0)) {
        return;
    }
