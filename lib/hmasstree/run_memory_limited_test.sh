#!/bin/bash
# H-Masstree Memory Limited Cold Restart Test
# 测试在严格内存限制下的冷启动性能
...
NUM_KEYS=${1:-1000000}
CACHE_SIZE_MB=${2:-16}
STORAGE_PATH="/tmp/hmasstree_memory_limited.dat"

echo "============================================"
echo "H-Masstree Memory Limited Test"
echo "============================================"
echo "Keys:       $NUM_KEYS"
...
# Create test program
cat > /tmp/memory_limited_test.cpp << 'EOFCPP'
#include <iostream>
#include <vector>
#include <chrono>
#include <algorithm>
...
# Compile with external storage support
echo "Compiling memory limited test..."
g++ -std=c++17 -O3 -DFLOWKV_KEY16 -DHMASSTREE_EXTERNAL_STORAGE \
    -I. -I.. -I../../include \
    -include config.h \
    /tmp/memory_limited_test.cpp \
    ...
    -lpthread -o /tmp/memory_limited_test 2>&1 | head -20

if [ ! -f /tmp/memory_limited_test ]; then
...
/tmp/memory_limited_test 1 "$STORAGE_PATH" $NUM_KEYS $CACHE_SIZE_MB
...
echo ""
echo "=== Simulating cold restart (process termination) ==="
sleep 1

echo ""
echo "=== Running Phase 2: Cold Restart & Benchmark ==="
/tmp/memory_limited_test 2 "$STORAGE_PATH" $NUM_KEYS $CACHE_SIZE_MB

echo ""
echo "============================================"
echo "Memory Limited Test Complete"
echo "============================================"
